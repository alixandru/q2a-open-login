 ...
 *
 * IMPORTANT!
 * This is a modified version of the library. Do not upgrade this file with a plain release of
 * LightOpenID because it will break the qa plugin.
 *
 *
 *
 * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * 
...
class LightOpenID
{
    ...
    function __construct($host, $cache = array())
    {
		...
		if(!empty($cache)) {
			...
			$this->server = $cache['endpoint'];
			$this->identifier_select = $cache['identifier_select'];
			$this->ax = $cache['xcap'][0];
			$this->sreg = $cache['xcap'][1];
		}
    }
    ...
    function __get($name)
    {
        ...
        case 'mode':
            $mode = empty($this->data['openid_mode']) ? null : $this->data['openid_mode'];
			if($mode == null) {
				/* for some reason the keys are separated by dot not underscores. fix this */
				$newdata = array();
				$data = $this->data;
